...
-- 注意: 既存データとの整合性が確認できた場合のみ実行
-- 必要に応じて以下のコメントアウトを外して実行

/*
-- user_roles テーブルの外部キー制約追加
...
-- role_permissions テーブルの外部キー制約追加
...
-- user_session_history テーブルの外部キー制約追加
ALTER TABLE user_session_history 
ADD CONSTRAINT fk_user_session_history_user_id 
FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;

-- password_resets テーブルの外部キー制約追加
ALTER TABLE password_resets 
...
-- ======== 実行完了メッセージ ========
SELECT 'UserService schema extension completed successfully!' as message;