...
## 第2節 Go-Zero Dashboard Service実装（契約駆動開発）

### 9.2.1 Dashboard Serviceの作成

Go-Zero契約駆動開発を使用してダッシュボード用サービスを構築します。

- [ ] プロジェクト構造の確認
```bash
cd /var/www/winyx/backend
mkdir dashboard_service
cd dashboard_service
```

### 9.2.2 API契約ファイルの設計

- [ ] ダッシュボード専用契約ファイル（CLAUDE.md規約準拠）
```bash
vim /var/www/winyx/contracts/dashboard_service/dashboard.api
```
...
### 9.2.3 Go-Zero契約駆動コード生成

- [ ] 契約ファイルからコード生成（CLAUDE.md厳守）
```bash
cd /var/www/winyx/backend/dashboard_service
goctl api go -api /var/www/winyx/contracts/dashboard_service/dashboard.api -dir . -style go_zero
```
...
```
dashboard_service/
├── /var/www/winyx/contracts/dashboard_service/dashboard.api  # API契約ファイル
├── dashboardservice.go            # メインエントリーポイント
├── etc/
│   └── dashboard_service-api.yaml  # 設定ファイル（CLAUDE.md規約）
├── internal/
│   ├── config/
│   │   └── config.go              # 設定構造体
│   ├── handler/                   # HTTPハンドラー
...
- [ ] 監視対象とデータベース設定を追加
```yaml
# etc/dashboard_service-api.yaml
...
const CONFIG = {
  apiFiles: [
    path.join(__dirname, '../backend/user_service/user.api'),
    path.join(__dirname, '../contracts/dashboard_service/dashboard.api'), // CLAUDE.md規約準拠
  ],
...
- [ ] Dashboard Serviceの起動（CLAUDE.md規約準拠）
```bash
cd /var/www/winyx/backend/dashboard_service
go run dashboardservice.go -f etc/dashboard_service-api.yaml
```
...
**契約駆動ダッシュボードオペレーション**により、Winyxプロジェクトの運用効率と安定性が大幅に向上します。Go-Zero契約駆動開発とNext.jsの連携によるリアルタイム監視システムは、CLAUDE.md規約に準拠したマイクロサービス環境での運用に不可欠な機能を提供します。
