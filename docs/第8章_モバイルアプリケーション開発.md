...
Winyxプロジェクトでは、**同一の契約ファイル**からWeb、モバイルすべてのクライアントコードを生成し、一貫性のある開発体験を提供します。
...
| クライアント | 開発環境 | 本番環境 | 特徴 |
|------------|----------|----------|------|
| **Next.js Web** | HTTP → REST API | API Routes → RPC | SSR対応、高速内部通信 |
| **Flutter Mobile** | HTTP → REST API | HTTP → REST API | クロスプラットフォーム |
| **React Native** | HTTP → REST API | HTTP → REST API | ネイティブ性能 |
| **iOS/Android** | HTTP → REST API | HTTP → REST API | 完全ネイティブ |
...
class AuthService {
  ...
  static String _getBaseUrl() {
    ...
    if (isProduction) {
      ...
      return 'http://10.0.2.2:8888'; // Android Emulator用
      // return 'http://localhost:8888'; // iOS Simulator用
    }
  }
...
- [ ] モバイル用設定の強化
```go
// backend/test_api/internal/config/config.go
type Config struct {
    rest.RestConf
...
- [ ] デバイス検出とバージョンチェック
```go
// backend/test_api/internal/middleware/mobile.go
...
```bash
# 1. 契約ファイルの編集
vim backend/test_api/test_api.api
...
# Ctrl+C で全て停止
trap 'kill $BACKEND_PID $FRONTEND_PID $FLUTTER_PID' EXIT
wait
```
...
**Contract-Driven Mobile Development**により、Web、モバイルすべてのクライアントが一貫した品質で開発できる環境が整いました。これにより、チーム全体での開発効率と品質の大幅な向上が実現されます。